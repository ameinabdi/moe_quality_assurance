...
class Dimension3Repository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    const record = await options.database.dimension3.create(
      {
        ...lodash.pick(data, [
          'indicator31',
          'indicator32',
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    let record = await options.database.dimension3.findOne(      
      {
        where: {
          id,
  ...
  static async findById(id, options: IRepositoryOptions) {
    ...
    const record = await options.database.dimension3.findOne(
      {
        where: {
          id,
          tenantId: currentTenant.id,
  ...
  static async filterIdsInTenant(
    ...
    const records = await options.database.dimension3.findAll(
      {
        attributes: ['id'],
        where,
      },
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return options.database.dimension3.count(
      {
        where: {
          ...filter,
          tenantId: tenant.id,
  ...
  static async findAndCountAll(
    ...
    let {
      rows,
      count,
    } = await options.database.dimension3.findAndCountAll({
      where,
      include,
...
export default Dimension3Repository;
