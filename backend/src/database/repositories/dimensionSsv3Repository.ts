...
class DimensionSsv3Repository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    const record = await options.database.dimensionSsv3.create(
      {
        ...lodash.pick(data, [
          'indicator51',
          'indicator52',
        ...
        schoolSupervisionId: data.supervision || null,
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    let record = await options.database.dimensionSsv3.findOne(      
      {
        where: {
          id,
        },
  ...
  static async findById(id, options: IRepositoryOptions) {
    ...
    const record = await options.database.dimensionSsv3.findOne(
      {
        where: {
          id,
        },
  ...
  static async filterIdsInTenant(
    ...
    const records = await options.database.dimensionSsv3.findAll(
      {
        attributes: ['id'],
        where,
      },
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return options.database.dimensionSsv3.count(
      {
        where: {
          ...filter,
          tenantId: tenant.id,
  ...
  static async findAndCountAll(
    ...
    let include = [
      ...
      {
        model: options.database.schoolSupervision,
    ...
    if (filter) {
      ...
      if (filter.supervision) {
        whereAnd.push({
          ['schoolSupervisionId']: SequelizeFilterUtils.uuid(filter.supervision),
        });
      }
    ...
    let {
      rows,
      count,
    } = await options.database.dimensionSsv3.findAndCountAll({
      where,
      include,
...
export default DimensionSsv3Repository;
