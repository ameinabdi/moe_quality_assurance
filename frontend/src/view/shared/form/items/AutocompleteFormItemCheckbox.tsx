...
import { Button, Form, Checkbox, Radio, Row, Col, Typography } from 'antd';
...
const AutocompleteFormItemCheckbox = (props) => {
  const {
    //@ts-ignore
    watch,
    setValue,
  ...
  const options = dataSource
    ...
    .map((d) => {
      return (
        <Col span={12}>
        <Checkbox key={d.id} value={d.id} checked>
          <div style={{}}>
            <div>{d.label}</div>
          </div>
        </Checkbox >
    ...
    >
      ...
      >
        ...
        onChange={(checkedValue: CheckboxValueType[]) => {
          const data:any = []
          checkedValue.map((value)=>{return  _.filter(dataSource,(item)=>item.id === value ? data.push(item) : undefined)})
          setValue(name, data, { shouldValidate: true, shouldDirty: true });
          props.onChange && props.onChange(data);
        }}
        defaultValue={originalValue}
        value={originalValue.map((value)=>value.id)}
        style={{
          width:'100%'
        }}
        >
...
AutocompleteFormItemCheckbox.defaultProps = {
  layout: null,
...
AutocompleteFormItemCheckbox.propTypes = {
...
export default AutocompleteFormItemCheckbox;
